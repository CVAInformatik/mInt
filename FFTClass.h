...
//
//  Wrapper around a FFTType pointer, when the application have many different length needs
//  ( like in Schönhage-Strassen multiplication ).
//
class FFTBufferType 
{
	public:  

	static const unsigned int DEFAULTLENGTH = 10000;
		
	FFTBufferType(unsigned int _len = DEFAULTLENGTH ){
		len = _len ;
		buffer = new FFTType[len];
	}
	...
	FFTType *setLength(unsigned int _len)	{
		  
		  if( len > _len){
	  	  if( buffer) delete [] buffer ;
				buffer = new FFTType[_len];	  
			}
			len = _len ;
			
			return ( FFTType *)  memset((void *) buffer, 0, len * sizeof(FFTType));
	}
